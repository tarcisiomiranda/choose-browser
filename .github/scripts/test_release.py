...
SPEC = importlib.util.spec_from_file_location("choose_browser_release", SCRIPT_PATH)
...
"""
    ...
    def test_build_body_from_notes(self) -> None:
        ...
        self.assertIn(
            "https://github.com/owner/choose-browser/compare/v0.9.0...v0.10.0",
            body,
        )
    ...
    def test_notes_file_preferred_over_commits(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            body = release.release_notes("v0.10.0", "owner/choose-browser", root=root)
...
class ReleaseBodyTests(unittest.TestCase):
    def test_groups_conventional_commits(self) -> None:
        ...
        self.assertIn(
            "https://github.com/owner/choose-browser/compare/v0.6.0...v0.7.0",
            body,
        )

    def test_returns_none_without_commits(self) -> None:
        self.assertIsNone(
            release.build_release_body([], "v0.6.0", "v0.7.0", "owner/choose-browser")
        )

    def test_first_release_has_no_compare_link(self) -> None:
        body = release.build_release_body(
            ["feat: initial"], None, "v0.1.0", "owner/choose-browser"
        )
        assert body is not None
    ...
    def test_skips_empty_sections(self) -> None:
        body = release.build_release_body(
            ["fix: only a fix"], "v0.6.0", "v0.7.0", "owner/choose-browser"
        )
        assert body is not None
